    ...
    public ResponseEntity<?> loginUser(@RequestBody User loginData) {
        ...
        if (userOpt.isPresent()) {
            ...
            if (matchesHashed || matchesPlainLegacy) {
                ...
                // Exception: admin accounts are provisioned out-of-band (direct DB access),
                // and "admin" is intentionally not a selectable frontend role option, so we
                // can't require an exact match for them - the password check is the real gate.
