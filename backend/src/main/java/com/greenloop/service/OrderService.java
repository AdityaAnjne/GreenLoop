...
public class OrderService {
    ...
    /**
     * Confirm order (PLACED → CONFIRMED) — SCOPED TO ONE RETAILER'S ITEMS
     */
    @Transactional
    public Order confirmOrder(Long orderId, Long retailerId, Long distributorId) throws Exception {
        Order order = orderRepository.findByIdWithItems(orderId)
                .orElseThrow(() -> new Exception("Order not found: " + orderId));

        List<OrderItem> myItems = order.getItems().stream()
                .filter(item -> retailerId.equals(item.getRetailerId()))
                .collect(Collectors.toList());

        if (myItems.isEmpty()) {
            throw new SecurityException("You can only confirm orders containing your own products");
        }

        List<OrderItem> confirmable = myItems.stream()
                .filter(item -> item.getStatus() == OrderStatus.PLACED)
                .collect(Collectors.toList());

        if (confirmable.isEmpty()) {
            throw new IllegalStateException("Your items in this order have already been confirmed");
        }

        // Retailer can assign to any registered distributor; record it for
        // traceability
        if (!retailerDistributorRepository.existsByRetailerIdAndDistributorId(retailerId, distributorId)) {
            retailerDistributorRepository.save(new RetailerDistributor(retailerId, distributorId));
        }

        // Only THIS retailer's own (still-PLACED) items move forward.
        // Other retailers' items in the same order are untouched and remain
        // visible/confirmable only on their own dashboards.
        for (OrderItem item : confirmable) {
    ...
    /**
     * Pack order items (CONFIRMED → PACKED) — SCOPED TO ONE DISTRIBUTOR'S ITEMS
     * Only the items a retailer assigned to THIS distributor are advanced.
     */
    @Transactional
    public Order packOrder(Long orderId, Long distributorId) throws Exception {
        ...
        List<OrderItem> myItems = order.getItems().stream()
                .filter(item -> distributorId.equals(item.getDistributorId()))
                .collect(Collectors.toList());
        ...
        List<OrderItem> packable = myItems.stream()
                .filter(item -> item.getStatus() == OrderStatus.CONFIRMED)
                .collect(Collectors.toList());

        if (packable.isEmpty()) {
            throw new IllegalStateException("Only CONFIRMED items can be packed");
        }
    ...
    /**
     * Ship order items (PACKED → SHIPPED) — SCOPED TO ONE DISTRIBUTOR'S ITEMS
     */
    @Transactional
    public Order shipOrder(Long orderId, Long distributorId) throws Exception {
        Order order = orderRepository.findByIdWithItems(orderId)
                .orElseThrow(() -> new Exception("Order not found: " + orderId));
        ...
        List<OrderItem> shippable = myItems.stream()
                .filter(item -> item.getStatus() == OrderStatus.PACKED)
                .collect(Collectors.toList());

        if (shippable.isEmpty()) {
            throw new IllegalStateException("Only PACKED items can be shipped");
        }
    ...
    /**
     * Deliver order items (SHIPPED → DELIVERED) — SCOPED TO ONE DISTRIBUTOR'S ITEMS
     */
    @Transactional
    public Order deliverOrder(Long orderId, Long distributorId) throws Exception {
        Order order = orderRepository.findByIdWithItems(orderId)
                .orElseThrow(() -> new Exception("Order not found: " + orderId));
        ...
        List<OrderItem> deliverable = myItems.stream()
    ...
    /**
     * Cancel order (whole order — customer/admin action, not retailer-scoped)
