import React, { useState, useEffect } from "react";
import { useTheme } from "../context/ThemeContext";
import {
  Package,
  Truck,
  TrendingUp,
  CheckCircle,
  Clock,
  AlertCircle,
} from "lucide-react";
import "../styles/DistributorDashboard.css";
import { getAuthHeaders, isTokenValid } from "../utils/tokenUtils";

const API_BASE_URL = process.env.REACT_APP_API_BASE_URL || "http://localhost:8080";

const DistributorDashboard = () => {
  const [orders, setOrders] = useState([]);
  const [activeTab, setActiveTab] = useState("orders");
  const { isDark } = useTheme();

  // Fetch distributor orders from backend
  const fetchDistributorOrders = async () => {
    try {
      if (!isTokenValid()) {
        setOrders([]);
        return;
      }

      const response = await fetch(
        `${API_BASE_URL}/api/orders/distributor`,
        {
          headers: getAuthHeaders(),
        },
      );

      if (!response.ok) {
        setOrders([]);
        return;
      }

      const data = await response.json();
      setOrders(Array.isArray(data) ? data : []);
    } catch (error) {
      console.error("Failed to fetch distributor orders:", error);
      setOrders([]);
    }
  };

  useEffect(() => {
    fetchDistributorOrders();
  }, []);

  // Update order status via backend API
  const updateOrderStatus = async (orderId, newStatus) => {
    try {
      if (!isTokenValid()) {
        alert("Please login to update order status");
        return;
      }

      let endpoint;
      if (newStatus === "PACKED") {
        endpoint = `${API_BASE_URL}/api/orders/${orderId}/pack`;
      } else if (newStatus === "SHIPPED") {
        endpoint = `${API_BASE_URL}/api/orders/${orderId}/ship`;
      } else if (newStatus === "DELIVERED") {
        endpoint = `${API_BASE_URL}/api/orders/${orderId}/deliver`;
      } else {
        // Other states not handled from this dashboard
        return;
      }

      const response = await fetch(endpoint, {
        method: "PUT",
        headers: getAuthHeaders(),
      });

      if (!response.ok) {
        const errorData = await response.json();
        alert(
          `Failed to update order: ${errorData.message || "Unknown error"}`,
        );
        return;
      }

      // Refresh orders after successful update
      fetchDistributorOrders();
    } catch (error) {
      console.error("Failed to update order status:", error);
      alert("Failed to update order status");
    }
  };

  const handleReceiveOrder = async (orderId) => {
    await updateOrderStatus(orderId, "DELIVERED");
  };

  const stats = [
    {
      label: "Total Orders",
      value: orders.length,
      icon: Package,
      color: "from-emerald-600 to-emerald-800",
    },
    {
      label: "Pending",
      value: orders.filter((o) => o.status === "CONFIRMED").length,
      icon: Clock,
      color: "from-harvest-400 to-harvest-600",
    },
    {
      label: "Shipped",
      value: orders.filter((o) => o.status === "SHIPPED").length,
      icon: Truck,
      color: "from-teal-500 to-emerald-700",
    },
    {
      label: "Delivered",
      value: orders.filter((o) => o.status === "DELIVERED").length,
      icon: CheckCircle,
      color: "from-emerald-500 to-emerald-700",
    },
  ];

  const getStatusIcon = (status) => {
    switch (status) {
      case "PLACED":
        return <Clock size={16} className="inline mr-1" />;
      case "CONFIRMED":
      case "PACKED":
        return <Package size={16} className="inline mr-1" />;
      case "SHIPPED":
        return <Truck size={16} className="inline mr-1" />;
      case "DELIVERED":
        return <CheckCircle size={16} className="inline mr-1" />;
      default:
        return <Package size={16} className="inline mr-1" />;
    }
  };

  return (
    <div className="distributor-dashboard">
      {/* Header */}
      <header className="distributor-header">
        <div className="distributor-header-container">
          <h1>Distributor Dashboard</h1>
          <p>Manage orders assigned to you</p>
        </div>
      </header>

      {/* Main Content */}
      <main className="distributor-main">
        {/* Stats Grid */}
        <div className="stats-grid">
          {stats.map((stat, idx) => {
            const Icon = stat.icon;
            return (
              <div key={idx} className="stat-card">
                <div className="stat-header">
                  <p className="stat-label">{stat.label}</p>
                  <div className={`stat-icon bg-gradient-to-br ${stat.color}`}>
                    <Icon size={24} />
                  </div>
                </div>
                <p className="stat-value">{stat.value}</p>
              </div>
            );
          })}
        </div>

        {/* Tabs */}
        <div className="dashboard-tabs">
          {[
            { id: "orders", label: "Orders", count: orders.length },
            { id: "analytics", label: "Analytics" },
          ].map((tab) => (
            <button
              key={tab.id}
              onClick={() => setActiveTab(tab.id)}
              className={`tab-button ${activeTab === tab.id ? "active" : ""}`}
            >
              {tab.label} {tab.count && `(${tab.count})`}
            </button>
          ))}
        </div>

        {/* Orders Tab */}
        {activeTab === "orders" && (
          <div className="dashboard-table-container">
            <div className="overflow-x-auto">
              <table className="dashboard-table">
                <thead>
                  <tr>
                    <th>Order ID</th>
                    <th>Items</th>
                    <th>Total</th>
                    <th>Date</th>
                    <th>Status</th>
                    <th>Action</th>
                  </tr>
                </thead>
                <tbody>
                  {orders.length === 0 ? (
                    <tr>
                      <td colSpan="6">
                        <div className="empty-state">
                          <div className="empty-state-icon">
                            <AlertCircle size={40} />
                          </div>
                          <p className="empty-state-text">
                            No orders assigned to you yet
                          </p>
                        </div>
                      </td>
                    </tr>
                  ) : (
                    orders.map((order) => (
                      <tr key={order.id}>
                        <td className="table-id">#{order.id}</td>
                        <td className="table-muted">
                          {order.items?.map((item, idx) => (
                            <div key={idx}>
                              {item.productName} ×{item.quantity}
                            </div>
                          ))}
                        </td>
                        <td className="table-id">
                          ₹{Number(order.totalAmount || 0).toFixed(2)}
                        </td>
                        <td className="table-muted">
                          {order.createdAt
                            ? new Date(order.createdAt).toLocaleDateString()
                            : "-"}
                        </td>
                        <td>
                          <span
                            className={`status-badge status-${
                              order.status?.toLowerCase() || "placed"
                            }`}
                          >
                            {getStatusIcon(order.status)}
                            {order.status}
                          </span>
                        </td>
                        <td>
                          {order.status === "CONFIRMED" && (
                            <button
                              onClick={() =>
                                updateOrderStatus(order.id, "PACKED")
                              }
                              className="action-btn btn-process"
                            >
                              Pack
                            </button>
                          )}
                          {order.status === "PACKED" && (
                            <button
                              onClick={() =>
                                updateOrderStatus(order.id, "SHIPPED")
                              }
                              className="action-btn btn-ship"
                            >
                              Ship
                            </button>
                          )}
                          {order.status === "SHIPPED" && (
                            <button
                              onClick={() => handleReceiveOrder(order.id)}
                              className="action-btn btn-fulfill"
                            >
                              Deliver
                            </button>
                          )}
                          {order.status === "PLACED" && (
                            <span className="table-muted">
                              Awaiting retailer confirmation
                            </span>
                          )}
                        </td>
                      </tr>
                    ))
                  )}
                </tbody>
              </table>
            </div>
          </div>
        )}

        {/* Analytics Tab */}
        {activeTab === "analytics" && (
          <div className="analytics-grid">
            <div className="analytics-card">
              <div className="analytics-header">
                <div className="analytics-icon bg-gradient-to-br from-emerald-600 to-emerald-800">
                  <Package size={24} />
                </div>
                <p className="analytics-label">Total Orders</p>
              </div>
              <p className="analytics-value">{orders.length}</p>
            </div>

            <div className="analytics-card">
              <div className="analytics-header">
                <div className="analytics-icon bg-gradient-to-br from-harvest-400 to-harvest-600">
                  <TrendingUp size={24} />
                </div>
                <p className="analytics-label">Total Value Handled</p>
              </div>
              <p className="analytics-value">
                ₹
                {orders
                  .reduce((sum, o) => sum + Number(o.totalAmount || 0), 0)
                  .toFixed(2)}
              </p>
            </div>

            <div className="analytics-card">
              <div className="analytics-header">
                <div className="analytics-icon bg-gradient-to-br from-green-500 to-emerald-600">
                  <CheckCircle size={24} />
                </div>
                <p className="analytics-label">Completion Rate</p>
              </div>
              <p className="analytics-value">
                {orders.length > 0
                  ? Math.round(
                      (orders.filter((o) => o.status === "DELIVERED").length /
                        orders.length) *
                        100,
                    )
                  : 0}
                %
              </p>
            </div>
          </div>
        )}
      </main>
    </div>
  );
};

export default DistributorDashboard;